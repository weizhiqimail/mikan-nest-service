...
export class AccountCommonService {
  ...
  async saveAccountUserToken<T = any>(key: string, userInfo: T) {
    return this.redisService.set(this.createAccountKey(key), userInfo, 60 * 60 * 24 * 3);
  }
  ...
  async setUserPassword(userId: string, password: string) {
    ...
    const userAccountExistResult = await this.userModelService.queryById(userId);
    
    const userAccountData = userAccountExistResult.data;
    if (userAccountData) {
      const userAccountUpdateResult = await this.accountModelService.updateById(userAccountData.id, { password: hashPassword });
    ...
    const createResult = await this.accountModelService.create({ userId, password: hashPassword });
